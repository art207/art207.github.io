...
│   └── site_stats.yml      small hand-edited numbers ("writing since")
...
├── blog/index.html         blog listing — auto-generated from _posts, with tag filtering
...
## 2. Point the sync indicator at your repo
...
This powers the small "synced ⟨date⟩" indicator in the header (latest commit date, pulled live from the GitHub API). It falls back to a static label if the API call fails, so nothing looks broken before you've filled this in.
...
tags: ["Data Engineering", "SQL"]   # optional — powers the filter on the blog page
readtime: "5 min read"
...
## 6. Blog tags and filtering

Any `tags:` you set in a post's front matter (see above) automatically:
- show as small pills under that post on the blog listing page and at the top of the post itself, and
- populate a filter-chip row at the top of `blog/index.html` — click a tag to show only posts with it, click "All" to reset.

Tags are open-ended (whatever you type), so they're styled as neutral pills rather than colored like `kind` — `kind` is a fixed, closed vocabulary (post/docs/project/incident/build) used for the color-coded chips/timeline-dots across the site, tags are a separate, free-form topic system. Reuse existing tag names where they fit (check the blog page for what's already in use) rather than inventing near-duplicates.

## 7. Admin tools (post generator)

`admin/new-post.html` is a form: pick "blog post" or "activity entry", fill in the fields, hit **Generate**, and it produces the exact Markdown + front matter file to save. After generating, it shows exactly what to do next: copy or download the file, create that file path on GitHub, paste the content in, and commit.

**The "Admin" link is intentionally not in the site's public navigation** — it isn't real security (see below), just kept out of casual visitors' way. Bookmark `/admin/login.html` directly.
...
## 8. Embed a Power BI report (or any iframe)
...
- The GitHub API (used for the sync-status indicator) is unauthenticated and rate-limited (60 requests/hour per visitor IP), which is plenty for a personal site with light traffic.
